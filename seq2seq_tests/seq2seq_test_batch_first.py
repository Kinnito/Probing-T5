...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, hid_dim, n_layers, dropout):
        ...
        self.rnn = nn.LSTM(emb_dim, hid_dim, n_layers, batch_first=True)#dropout = dropout)#, batch_first=True)
    ...
    def forward(self, inp, hidden, cell):
        ...
        #print("cell:", cell)
        #pdb.set_trace()
        output, (hidden, cell) = self.rnn(embedded, (hidden.detach(), cell.detach()))
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio = 0.5):
        ...
        for t in range(1, trg_len):
            start = time.time()
            output, hidden, cell = self.decoder(inp.squeeze(), hidden, cell)
        ...
        #print("decoder elapsed time:", end-start)
        return outputs
...
def train(model, train_dat, train_label, dev_dat, dev_label):
    ...
    for idx, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #print("elapsed time in one train loop:", end-start)
            #print("label shape:", labels.shape)
            labels = labels[1:].reshape(-1)
            #print("output shape after:", output.shape)
            #print("label shape after:", labels.shape)
...
if __name__ == "__main__":
    ...
    if args.mode:
        ...
        train_data = []
        train_label = []
        train_data.append(f_train['1000'][()])
        train_label.append(f_train_label['1000'][()])
        train_data.append(f_train['2000'][()])
        train_label.append(f_train_label['2000'][()])
